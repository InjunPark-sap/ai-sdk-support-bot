...
const AGENT_SYSTEM_PROMPT = [
  ...
  '- semantic_search       — search local pre-built issue embedding index by semantic similarity.',
  '  Returns top-5 issues. Use AFTER github__search_issues to cross-validate:',
  '  if both return the same issue number, confidence is HIGH.',
  '  If they diverge, note the discrepancy and prefer the source with more detail.',
  '',
  ...
  '3. Call semantic_search with the same query to cross-validate GitHub MCP results.',
  '4. If search results contain issues closely matching the problem (same API, same error type),',
  '   call github__get_issue for at most 3 of them to get full details.',
  '5. Answer based ONLY on what you retrieved. Do not invent API method names or issue numbers.',
  '',
  '## Dynamic source retrieval — use when steps 1-4 do not yield a definitive answer',
...
export async function initAgent(): Promise<void> {
  ...
  // semantic_search: local embedding index as a first-class tool for cross-validation with GH MCP
  const semanticTool = new DynamicTool({
    name: 'semantic_search',
    description:
      'Search the local pre-built issue embedding index by semantic similarity. ' +
      'Returns top 5 most similar issues (resolved issues weighted). ' +
      'Use to cross-validate github__search_issues results.',
    func: async (query: string) => {
      const results = await semanticSearch(query);
      if (!results.length) return 'No semantically similar issues found.';
      return results
        .map(i => '#' + i.number + ' [' + i.state + '] ' + esc(i.title) +
          (i.resolution ? ' → ' + esc(i.resolution) : ''))
        .join('\n');
    }
  });

  tools = [...mcpTools, semanticTool as unknown as StructuredToolInterface];
  ...
  console.error(`  local     semantic_search`);
  loadEmbeddingIndex();
}
